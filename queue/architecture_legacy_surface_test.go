...
func TestArchitectureRemovesLegacyQueueSurfaces(t *testing.T) {
	...
	assertFileContains(t, filepath.Join(root, "manager.go"), "func (m *Manager) Extend(")
	assertFileNotContains(t, filepath.Join(root, "facade.go"), "func DispatchEnvelope(")
	assertFileContains(t, filepath.Join(root, "facade.go"), "Resolve().Extend(")
	...
	assertPathMissing(t, filepath.Join(root, "rabbitmq"))
